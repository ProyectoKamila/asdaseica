<?php require_once('Connections/conexion.php'); if (!function_exists("GetSQLValueString")) { function GetSQLValueString($theValue, $theType, $theDefinedValue = "", $theNotDefinedValue = "") { if (PHP_VERSION < 6) { $theValue = get_magic_quotes_gpc() ? stripslashes($theValue) : $theValue; } $theValue = function_exists("mysql_real_escape_string") ? mysql_real_escape_string($theValue) : mysql_escape_string($theValue); switch ($theType) { case "text":$theValue = ($theValue != "") ? "'" . $theValue . "'" : "NULL";break; case "long":case "int":$theValue = ($theValue != "") ? intval($theValue) : "NULL"; break;case "double":$theValue = ($theValue != "") ? doubleval($theValue) : "NULL";break;case "date":$theValue = ($theValue != "") ? "'" . $theValue . "'" : "NULL"; break;  case "defined": $theValue = ($theValue != "") ? $theDefinedValue : $theNotDefinedValue; break;} return $theValue;}}$pg = 4;mysql_select_db($database_conexion, $conexion);$query_titulo = "SELECT * FROM tblobjetivos_titu WHERE tblobjetivos_titu.intEstado = 1";$titulo = mysql_query($query_titulo, $conexion) or die(mysql_error());$row_titulo = mysql_fetch_assoc($titulo);$totalRows_titulo = mysql_num_rows($titulo);?>
<!doctype html>
<html>
<head>
...
<div class="container">
  ...
  <div class="intro-content-wrap col-xs-12 col-sm-12 col-md-12 col-lg-12 pull-left no-pad wow fadeInUp animated" data-wow-delay="1.5s" data-wow-offset="110">
    
  <div class="col-xs-12 col-sm-4 col-md-6 col-lg-6 no-pad">
  <img class="img-responsive pull-left" alt="<?php echo $row_titulo['strTutulo']; ?>" src="imagenes/objetivos/<?php echo $row_titulo['txtImagen']; ?>"/>
  </div>
    
  <div class="col-xs-12 col-sm-8 col-md-6 col-lg-6 no-pad padin_inter">
  <p><?php echo $row_titulo['strTutulo']; ?></p>
  <?php echo $row_titulo['txtPar_Cor']; mysql_select_db($database_conexion, $conexion);$query_listas = sprintf("SELECT * FROM tblobjetivos_lista WHERE tblobjetivos_lista.intObj_titu = %s ORDER BY tblobjetivos_lista.idLista", $row_titulo['idObjetivo']);$listas = mysql_query($query_listas, $conexion) or die(mysql_error());$row_listas = mysql_fetch_assoc($listas);$totalRows_listas = mysql_num_rows($listas);?>
  <ul>
  <?php do { ?>
  <li><i class="fa fa-angle-right about-list-arrows"></i><?php echo $row_listas['strObjetivo']; ?></li>
